...
function main() {
  console.log('🧹 Полная очистка generateHrefLangs из всех layout файлов...\n');
  ...
  const otherLayouts = [
    ...
    path.join(__dirname, '../../app/[locale]/blog/[slug]/layout.tsx'),
  ];

  otherLayouts.forEach(layoutPath => {
    if (fs.existsSync(layoutPath)) {
      cleanupFile(layoutPath);
    }
  });

  console.log('\n✅ Очистка generateHrefLangs завершена!');
}
